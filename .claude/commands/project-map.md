...
3. Установи Semgrep — опциональный, но рекомендуемый движок индексации
   (точнее извлекает классы/функции/эндпоинты, поддерживает Go и C#).
   Если пакет не установится — индексатор автоматически откатится на
   встроенный Tree-sitter, граф всё равно построится:
   ```
   pip install -r indexer/requirements-semgrep.txt || echo "Semgrep недоступен — будет использован Tree-sitter"
   ```
