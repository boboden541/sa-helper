...
- Техническое имя метода/функции указывается в скобках (например: `ShowManage::getById`).
...
- **Слой данных:** Обязательно указывать логическое имя хранилища (MSSQL, Sphinx, uat_db) и конкретный объект (View, Table, Index).
