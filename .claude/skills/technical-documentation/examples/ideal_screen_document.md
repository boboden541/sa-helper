...
> **Назначение шаблона.** Документ типа **Screen** («Описание экранов») — это обзорная карта одного экрана (страницы) приложения и точка входа в его документацию. Он НЕ дублирует детальное описание API-методов: каждый метод описывается в собственном документе типа API (папка `api/`, соседняя с `screens/` внутри `<сервис>-docs/`), а здесь приводится только сводная карта методов со ссылками. Структура ниже обязательна; разделы не удалять.
...
> Перечисли ВСЕ API-методы, вызываемые на экране. Собственные методы экрана — со ссылкой на их документ в `api/` (относительный путь из `screens/`: `../api/{api_файл}.md`). Переиспользуемые методы (reuse) вынеси в отдельную таблицу ниже с указанием владельца.

| # | Метод | Назначение | Контроллер | Документ |
|---|---|---|---|---|
| `{ID.1}` | `{METHOD} {URL}` | `{назначение}` | `{Controller::action}` | [{api_файл}.md](../api/{api_файл}.md) |
| `{ID.2}` | `{METHOD} {URL}` | `{назначение}` | `{Controller::action}` | [{api_файл}.md](../api/{api_файл}.md) |

**Переиспользуемые методы (reuse, документируются у владельца):**

| Метод | Назначение | Контроллер | Документ | Владелец |
|---|---|---|---|---|
| `{METHOD} {URL}` | `{назначение}` | `{Controller::action}` | [{api_файл}.md](../api/{api_файл}.md) | `{ID владельца}` |
