...
| `{HEADER_NAME}` | `{HEADER_TYPE}` | `{required/optional}` |`{HEADER_DESCRIPTION}` | `{HEADER_EXAMPLE}` | `{HEADER_COMMENT}` | `{HEADER_MAPPING}` |
| `{PARAM_NAME}` | `{PARAM_TYPE}` | `{required/optional}` | `{PARAM_DESCRIPTION}` | `{Внешнаяя интеграция/Атрибут вычисляется/Итд}` | `{PARAM_EXAMPLE}` | `{API_method + param_name} или {TABLE_NAME.FIELD_NAME} или {METHOD_NAME}` |
| `{NESTED_OBJECT_NAME}` | `Object` / `Array<{ObjectName}>` | `{required/optional}` | `{OBJECT_DESCRIPTION}` | `{...}` / `[{...}]` | См. подтаблицу ниже | `—` |

> **Типизация массивов (обязательно):** тип любого параметра-массива обязан указывать тип элемента — `Array<{ObjectName}>` для массива объектов (где `{ObjectName}` = имя DTO/класса из кода либо описательное имя элемента) или `Array<{scalar}>` для массива скаляров (`Array<string>`, `Array<int>`). Голый `array` / `list` без параметра типа элемента **запрещён**. Для `Array<{ObjectName}>` имя `{ObjectName}` обязано совпадать с заголовком парной подтаблицы «Описание вложенного {ObjectName}». Вырожденные параметры (`Array<>`, `Array<mixed>`) для массива объектов недопустимы.

> **Вложенные объекты:** если поле ответа имеет тип `Object` или `Array<{ObjectName}>`, оно указывается в основной таблице как контейнер (Origin = `—`), а его внутренние поля раскрываются ниже в отдельной подтаблице `#### Описание вложенного {ObjectName}`. Это сохраняет иерархию и читаемость Origin-маппинга. Для простых ответов (без вложенных объектов) подтаблицы не создаются.
...
| `{NESTED_OBJECT_NAME_2}` | `Object` | `{required/optional}` | `{OBJECT_DESCRIPTION}` | `{...}` | См. подтаблицу `Описание вложенного {ObjectName_2}` |
...
> **Типизация массивов (обязательно):** тип любого параметра-массива обязан указывать тип элемента — `Array<{ObjectName}>` для массива объектов или `Array<{scalar}>` для массива скаляров. Голый `array` / `list` без параметра типа элемента **запрещён**. Вырожденные параметры (`Array<>`, `Array<mixed>`) для массива объектов недопустимы.

> **Вложенные объекты:** если ответ внешнего сервиса содержит поле типа `Object` или `Array<{ObjectName}>`, оно указывается в таблице выше как контейнер (Origin = `—`), а его внутренние поля раскрываются ниже в подтаблице `##### Описание вложенного {ObjectName}`. Рекурсивно — для каждого вложенного объекта своя подтаблица. Для простых ответов подтаблицы не создаются.
