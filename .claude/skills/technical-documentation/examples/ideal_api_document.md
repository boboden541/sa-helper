...
| `{NESTED_OBJECT_NAME}` | `Object` / `Array<{ObjectName}>` | `{OBJECT_DESCRIPTION}` | `{...}` / `[{...}]` | См. подтаблицу ниже | `—` |

> **Вложенные объекты:** если поле ответа имеет тип `Object` или `Array<Object>`, оно указывается в основной таблице как контейнер (тип `Object` / `Array<{ObjectName}>`, Origin = `—`), а его внутренние поля раскрываются ниже в отдельной подтаблице `#### Описание вложенного {ObjectName}`. Это сохраняет иерархию и читаемость Origin-маппинга. Для простых ответов (без вложенных объектов) подтаблицы не создаются.
...
| `{NESTED_OBJECT_NAME_2}` | `Object` | `{OBJECT_DESCRIPTION}` | `{...}` | См. подтаблицу `Описание вложенного {ObjectName_2}` |
...
|----------|-----|-----------|--------|---------|
| `{PARAM_NAME}` | `{PARAM_TYPE}` | `{PARAM_DESCRIPTION}` | `{PARAM_EXAMPLE}` | `{TABLE_NAME.FIELD_NAME}` / `{API_NAME.JSON_PATH}` / `{METHOD_NAME}` |
| `{NESTED_OBJECT_NAME}` | `Object` / `Array<{ObjectName}>` | `{OBJECT_DESCRIPTION}` | `{...}` / `[{...}]` | `—` |

> **Вложенные объекты:** если ответ внешнего сервиса содержит поле типа `Object` или `Array<Object>`, оно указывается в таблице выше как контейнер (Origin = `—`), а его внутренние поля раскрываются ниже в подтаблице `##### Описание вложенного {ObjectName}`. Рекурсивно — для каждого вложенного объекта своя подтаблица. Для простых ответов подтаблицы не создаются.
