...
>
> *Каждый функциональный блок с задачами открывается чек-листом состава доработок (уровень Л1): обзор «что делаем» широкими мазками + самопроверка полноты. Правила формулировок — `resources/abstraction_levels.md`, раздел 2.*

### 4.1 <Название функционального блока>

**Состав доработок:**

- [ ] Добавляем в <контур> <метод / API / таблицу> — <результат широкими мазками> (4.N.M)
- [ ] Расширяем <существующий метод> — <что добавляется> (4.N.M)
- [ ] Меняем источник <чего>: <было> → <стало> (4.N.M)
- [ ] Переводим <экран / список> с <источника> на <источник> (4.N.M)
- [ ] Убираем <обращение / шаг> (4.N.M)

*Сквозные (чекбокс — в их собственном блоке): <журналирование — N.M, имя контрагента — N.M>.*

> Чекбокс закрывается при закрытии задачи. UI-задачи раздела 5, обслуживающие экран блока, включаются в этот список чекбоксами. Открытый вопрос, блокирующий пункт, помечается маркером прямо в пункте (`[ВОПРОС Б-k]` / `[NEEDS_INVESTIGATION]`). Неприменимые шаблоны пунктов удаляются.
...
*Сквозные (чекбокс — в их собственном блоке): <перечень> (N.M).*

> Чекбокс закрывается при закрытии задачи; правила формулировок — `resources/abstraction_levels.md`, раздел 2.
...
> *Служебная пометка уровней (удаляется при подготовке к согласованию): «Общее описание доработки» — уровень Л1, контекст для всей команды; «Описание доработок» — уровень Л2, постановка задачи для системного аналитика и разработчика. Полный контракт — `resources/abstraction_levels.md`.*
>
> *Каждый блок раздела с задачами открывается чек-листом состава доработок (уровень Л1) — обзор «что делаем» + самопроверка полноты. Правила — `resources/abstraction_levels.md`, раздел 2.*
...
- [ ] Добавляем <экран / элемент / диалог> — <результат широкими мазками> (5.N)
- [ ] Меняем <поведение экрана>: <было> → <стало> (5.N)

> Чекбокс закрывается при закрытии задачи; UI-задачи, обслуживающие экран backend-блока раздела 4, включаются чекбоксами и в список того блока. Правила формулировок — `resources/abstraction_levels.md`, раздел 2.
...
- **Каждый функциональный блок с задачами открывается чек-листом состава доработок** (в простом документе без блоков — один список после заголовка раздела с задачами): чекбокс-пункты «что делаем» широкими мазками, каждый пункт со ссылкой на задачу. Глагольные шаблоны — «Добавляем в <контур> …», «Расширяем …», «Меняем источник …», «Переводим …», «Убираем …»; жаргон («даём API», «прокидываем») запрещён. UI-задачи раздела 5 входят чекбоксами в списки своих экранов; сквозные задачи — строкой-примечанием без чекбокса. Чекбокс закрывается при закрытии задачи; открытые вопросы помечаются маркером прямо в пункте.
